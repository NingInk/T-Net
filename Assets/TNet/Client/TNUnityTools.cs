...
namespace TNet
{
    ...
    static public class UnityTools
    {
        ...
        static public GameObject LoadPrefab(string path)
        {
            Debug.Log(path);
            if (string.IsNullOrEmpty(path)) return null;
            if (!UnityEngine.Application.isPlaying) return Resources.Load(path, typeof(GameObject)) as GameObject;
        ...
        {
            if (UnityEngine.Application.isPlaying) UnityEngine.Object.Destroy(obj);
        ...
        static public Texture2D ReadPNG(string path, Texture2D existing = null)
        {
            ...
            if (bytes != null)
            {
                ...
                if (existing == null)
                {
                    if (UnityEngine.Application.isPlaying) Object.Destroy(tex);
